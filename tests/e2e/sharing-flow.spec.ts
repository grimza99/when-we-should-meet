...
test.describe("공유 플로우", () => {
  test("초대 코드, 방 링크, 랭킹 문구를 클립보드 폴백으로 복사한다", async ({
    ...
    try {
      ...
      const inviteCode =
        (
          await page
            .locator(byAriaLabel(ARIA_LABELS.room.inviteCodeHeading))
            .textContent()
        )?.trim() || "";
      const roomUrl = page.url();

      await page
        .locator(byAriaLabel(ARIA_LABELS.room.copyInviteCodeButton))
        .click();
  ...
  test("브라우저 공유 API가 있으면 navigator.share를 사용한다", async ({
    browser,
  }) => {
