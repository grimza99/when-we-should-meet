...
test.describe("카카오 공유 플로우", () => {
  test("방 공유와 랭킹 공유를 요청하면 카카오 SDK를 호출한다", async ({
    browser,
  }) => {
    ...
    try {
      ...
      expect(allShareCalls[1]?.text).toContain(
        "우리 언제 볼까? 일정 랭킹이에요."
      );
...
async function getKakaoShareCalls(page: Page) {
  return page.evaluate(() => {
    return (
      (window as Window & { __kakaoShareCalls?: KakaoShareCall[] })
        .__kakaoShareCalls ?? []
    );
  });
}
...
async function getKakaoInitState(page: Page) {
  return page.evaluate(() => {
    return (
      (window as Window & { __kakaoInitState?: KakaoInitState })
        .__kakaoInitState ?? {
        calls: 0,
        key: "",
      }
